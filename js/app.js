import { auth, db, GoogleAuthProvider, signInWithPopup, onAuthStateChanged, signOut, collection, addDoc, query, orderBy, getDocs, serverTimestamp, doc, updateDoc, limit } from './firebase.js'
...
tagInput.addEventListener('keydown', (e)=>{ if(e.key==='Enter'&&tagInput.value.trim()){ tags.add(tagInput.value.trim()); tagInput.value=''; renderTags() } })
codeInput.addEventListener('input', ()=>{ analyzeCode(codeInput.value) })
function showView(view){ document.querySelectorAll('.nav button').forEach(b=>b.classList.remove('active')); if(view==='new') document.getElementById('newBtn').classList.add('active'); if(view==='lib') document.getElementById('libraryBtn').classList.add('active'); if(view==='mine') document.getElementById('mySnippetsBtn').classList.add('active'); renderLibrary() }
function toggleTheme(){ document.documentElement.classList.toggle('light'); if(document.documentElement.classList.contains('light')){ document.body.style.background='linear-gradient(180deg,#f7fbff,#e6eef8)'; document.body.style.color='#0b1220' } else { document.body.style.background='linear-gradient(180deg,#02040a,#0b0f14)'; document.body.style.color='#e6eef8' } }
...
function loadSnippetIntoEditor(s){ titleInput.value = s.title; codeInput.value = s.content; langSelect.value = s.lang || 'javascript'; tags = new Set(s.tags || []); renderTags(); editingId = s.id; analyzeCode(s.content); explainBox.style.display='none' }
async function forkSnippet(s){ if(!currentUser){ alert('Sign in to fork'); return } const copy = { ...s, title: s.title + ' (fork)', owner: currentUser.uid, ownerName: currentUser.displayName, public: false, createdAt: serverTimestamp() }; delete copy.id; try{ await addDoc(collection(db, 'snippets'), copy); alert('Forked into your snippets'); renderLibrary() } catch(e){ alert(e.message) } }
async function googleSignIn(){ const provider = new GoogleAuthProvider(); signInWithPopup(auth, provider).then(result=>{ currentUser = result.user; onAuthChange(currentUser) }).catch(e=>alert(e.message)) }
function onAuthChange(user){ currentUser = user; const authArea = document.getElementById('authArea'); authArea.innerHTML = ''; if(user){ const img = document.createElement('img'); img.src = user.photoURL; img.style.width='36px'; img.style.height='36px'; img.style.borderRadius='6px'; img.style.marginRight='8px'; const name = document.createElement('span'); name.textContent = user.displayName; name.style.marginRight='8px'; const out = document.createElement('button'); out.className='btn secondary'; out.textContent='Sign out'; out.onclick = ()=>signOut(auth); authArea.appendChild(img); authArea.appendChild(name); authArea.appendChild(out) } else { const btn = document.createElement('button'); btn.className='btn'; btn.textContent='Sign in with Google'; btn.onclick = googleSignIn; authArea.appendChild(btn) } renderLibrary() }
onAuthStateChanged(auth, (u)=>{ if(u) onAuthChange(u); else onAuthChange(null) })
function renderTags(){ tagList.innerHTML=''; Array.from(tags).forEach(t=>{ const el=document.createElement('div'); el.className='tag'; el.textContent=t; el.onclick=()=>{ tags.delete(t); renderTags() }; tagList.appendChild(el) }) }
renderLibrary()
